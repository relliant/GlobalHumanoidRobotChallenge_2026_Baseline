...
from src.lerobot.envs.configs import EnvConfig
from src.lerobot.envs.utils import env_to_policy_features
from src.lerobot.policies.act.configuration_act import ACTConfig
from src.lerobot.policies.diffusion.configuration_diffusion import DiffusionConfig
from src.lerobot.policies.groot.configuration_groot import GrootConfig
from src.lerobot.policies.pi0.configuration_pi0 import PI0Config
from src.lerobot.policies.pi05.configuration_pi05 import PI05Config
...
from src.lerobot.policies.sarm.configuration_sarm import SARMConfig
from src.lerobot.policies.smolvla.configuration_smolvla import SmolVLAConfig
from src.lerobot.policies.tdmpc.configuration_tdmpc import TDMPCConfig
from src.lerobot.policies.utils import validate_visual_features_consistency
from src.lerobot.policies.vqbet.configuration_vqbet import VQBeTConfig
from src.lerobot.policies.wall_x.configuration_wall_x import WallXConfig
from src.lerobot.policies.xvla.configuration_xvla import XVLAConfig
from src.lerobot.processor import PolicyAction, PolicyProcessorPipeline
from src.lerobot.processor.converters import (
    batch_to_transition,
...
def get_policy_class(name: str) -> type[PreTrainedPolicy]:
    ...
    """
    if name == "tdmpc":
        from src.lerobot.policies.tdmpc.modeling_tdmpc import TDMPCPolicy
        ...
        from src.lerobot.policies.vqbet.modeling_vqbet import VQBeTPolicy

        return VQBeTPolicy
    elif name == "pi0":
        from src.lerobot.policies.pi0.modeling_pi0 import PI0Policy

        return PI0Policy
    elif name == "pi0_fast":
        from src.lerobot.policies.pi0_fast.modeling_pi0_fast import PI0FastPolicy
        ...
        from src.lerobot.policies.sac.reward_model.modeling_classifier import Classifier
        ...
        from src.lerobot.policies.sarm.modeling_sarm import SARMRewardModel

        return SARMRewardModel
    elif name == "groot":
        from src.lerobot.policies.groot.modeling_groot import GrootPolicy

        return GrootPolicy
    elif name == "xvla":
        from src.lerobot.policies.xvla.modeling_xvla import XVLAPolicy

        return XVLAPolicy
    elif name == "wall_x":
        from src.lerobot.policies.wall_x.modeling_wall_x import WallXPolicy
...
def make_pre_post_processors(
    ...
    **kwargs: Unpack[ProcessorConfigKwargs],
    ...
    if isinstance(policy_cfg, TDMPCConfig):
        from src.lerobot.policies.tdmpc.processor_tdmpc import make_tdmpc_pre_post_processors
        ...
        from src.lerobot.policies.diffusion.processor_diffusion import make_diffusion_pre_post_processors
        ...
        from src.lerobot.policies.act.processor_act import make_act_pre_post_processors
        ...
        from src.lerobot.policies.vqbet.processor_vqbet import make_vqbet_pre_post_processors
        ...
        from src.lerobot.policies.pi0.processor_pi0 import make_pi0_pre_post_processors
        ...
        from src.lerobot.policies.pi05.processor_pi05 import make_pi05_pre_post_processors
        ...
        from src.lerobot.policies.sac.processor_sac import make_sac_pre_post_processors
        ...
        from src.lerobot.policies.sac.reward_model.processor_classifier import make_classifier_processor
        ...
        from src.lerobot.policies.smolvla.processor_smolvla import make_smolvla_pre_post_processors
        ...
        from src.lerobot.policies.sarm.processor_sarm import make_sarm_pre_post_processors
        ...
        from src.lerobot.policies.groot.processor_groot import make_groot_pre_post_processors
        ...
        from src.lerobot.policies.wall_x.processor_wall_x import make_wall_x_pre_post_processors
...
def make_policy(
    ...
    # Truncate observation.state to 20 dims to match model's max_state_dim constraint
    # (e.g. pi0/smolvla/pi05 use max_state_dim=32, but dataset may have more dims
    #  from object poses etc.). The corresponding data slicing is handled by
    #  StateSlicerProcessorStep in the processor pipeline.
    if OBS_STATE in features and features[OBS_STATE].shape[0] > 20:
        features[OBS_STATE] = PolicyFeature(
            type=features[OBS_STATE].type,
            shape=(20,),
        )
    ...
    # For policies that use visual features (e.g., smolvla), the pretrained config may encode
    # different camera keys/counts than the dataset. Since the model architecture processes
    # images dynamically (no fixed camera count), update input_features with dataset visuals
    # so that prepare_images() and validation see the actual dataset camera keys.
    # NOTE: This block covers the non-pretrained path. For the pretrained path, the same
    # update must be re-applied AFTER from_pretrained (see below), because from_pretrained()
    # re-parses the config from disk and discards any in-memory cfg.input_features changes.
    if cfg.pretrained_path:
        dataset_visuals = {k: v for k, v in features.items() if v.type is FeatureType.VISUAL}
        if dataset_visuals:
            cfg.input_features = {
                k: v for k, v in cfg.input_features.items() if v.type is not FeatureType.VISUAL
            }
            cfg.input_features.update(dataset_visuals)
    ...
    if cfg.pretrained_path and not cfg.use_peft:
        ...
        # Re-apply dataset visual features after from_pretrained re-parses config from disk
        dataset_visuals = {k: v for k, v in features.items() if v.type is FeatureType.VISUAL}
        if dataset_visuals:
            policy.config.input_features = {
                k: v for k, v in policy.config.input_features.items() if v.type is not FeatureType.VISUAL
            }
            policy.config.input_features.update(dataset_visuals)
            # Sync cfg.input_features — validation checks cfg, which may be a different object
